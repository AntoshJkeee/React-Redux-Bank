            ...
            <div className="result">
                { customers.length ?
                        customers.map((customer) => {
                            return <div className='user-info' key={customer.id}><h3>Пользователь: </h3><div className='group'><h3>{customer.name}</h3><AiFillCloseCircle onClick={() => removeCustomer(customer)} className='ico'/></div></div>
                        }
                        )
                        :
